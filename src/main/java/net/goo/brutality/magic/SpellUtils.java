...
public class SpellUtils {
    ...
    private static final float SCRIBE_MAX_MULT = 0.25F, SCRIBE_NOT_MAX_MULT = -0.1F;
    private static final float EMERGENCY_FLASK_CAST_TIME_MULT = -0.33F;
...
//        return multiplier[0];
    ...
    public static float getCurioCooldownMultiplier(@Nullable Entity entity, @Nullable IBrutalitySpell spell, @Nullable Integer spellLevel) {
        ...
        CuriosApi.getCuriosInventory(caster).ifPresent(handler -> {
            handler.findFirstCurio(BrutalityModItems.ECHO_CHAMBER.get()).ifPresent(slot -> {
