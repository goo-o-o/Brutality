    ...
    public boolean onStartCast(Player player, ItemStack stack, int spellLevel) {
        ...
        if (player.level() instanceof ServerLevel serverLevel) {
            for (double d = 0; d < distance; d += 0.5F) {
                ...
                ModUtils.sendParticles(serverLevel, TerramityModParticleTypes.ANTIMATTER.get(), true, particlePos, 0, 0, 0, 0, 1);
            }
            ModUtils.sendParticles(serverLevel, TerramityModParticleTypes.ANTIMATTER.get(), true, target, target.getBbWidth() / 2,
