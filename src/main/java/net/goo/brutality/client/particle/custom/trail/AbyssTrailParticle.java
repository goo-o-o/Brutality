...
public class AbyssTrailParticle extends AbstractHeadedTrailParticle {
    private final Entity entity;

    protected AbyssTrailParticle(EntityIdParticleData<?> data, ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
        super(level, x, y, z, xSpeed, ySpeed, zSpeed);
        this.alpha = 1.0F;
        this.hasPhysics = false;
        entity = level.getEntity(data.getEntityId());
        this.lifetime = 1000;
        this.setColor(0, 0.12941177f, 0.9529412f);
    }
    ...
    private static final int DARK_BLUE = FastColor.ARGB32.color(255, 2, 0, 121);
    private static final int BLUE = FastColor.ARGB32.color(255, 0, 33, 243);

    @Override
    public int getTrailColor(float percentageFromHead) {
        return FastColor.ARGB32.lerp(percentageFromHead, BLUE, DARK_BLUE);
    }
    ...
    public static class Provider implements ParticleProvider<EntityIdParticleData<?>> {
        public Particle createParticle(EntityIdParticleData<?> data, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            AbyssTrailParticle particle = new AbyssTrailParticle(data, worldIn, x, y, z, xSpeed, ySpeed, zSpeed);
            particle.trailR = 0;
            particle.trailG = 33;
            particle.trailB = 243;
            return particle;
