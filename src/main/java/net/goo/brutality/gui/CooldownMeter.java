...
public abstract class CooldownMeter implements IGuiOverlay {
    ...
    public enum AbilityCooldownPosition {
        ...
        LEFT {
            ...
            @Override
            public int getY(int screenHeight, int elementHeight) {
                return screenHeight / 2 - Constants.smallPad - elementHeight;
            }
        },
        RIGHT {
            @Override
    ...
    public enum ArmorSetCooldownPosition {
        BOTTOM_LEFT {
            @Override
            public int getX(int screenWidth, int elementWidth) {
                return Constants.bigPad + elementWidth + Constants.bigPad;
            }
        ...
        BOTTOM_RIGHT {
            @Override
            public int getX(int screenWidth, int elementWidth) {
                return screenWidth - Constants.bigPad - elementWidth - elementWidth - Constants.bigPad;
            }
        ...
        LEFT {
            ...
            @Override
            public int getY(int screenHeight, int elementHeight) {
                return screenHeight / 2 + Constants.smallPad;
            }
        },
        RIGHT {
            @Override
        ...
        HOTBAR_RIGHT {
            @Override
            public int getX(int screenWidth, int elementWidth) {
                return screenWidth / 2 + 91 + Constants.bigPad + elementWidth + Constants.smallPad;
            }
        ...
        HOTBAR_LEFT {
            @Override
            public int getX(int screenWidth, int elementWidth) {
                return screenWidth / 2 - 91 - Constants.bigPad - elementWidth - elementWidth - Constants.bigPad;
            }
    ...
    public static CooldownMeter get(Constants.CooldownType type) {
        Supplier<CooldownMeter> sup = Constants.REGISTRY.get(type);
    ...
    public static class ArmorSetAbilityCooldownMeter extends CooldownMeter {
        ...
        public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
            ...
            int x = pos.getX(guiGraphics.guiWidth(), Constants.itemWidth);
            int y = pos.getY(guiGraphics.guiHeight(), Constants.itemHeight);

            if (pos == ArmorSetCooldownPosition.HOTBAR_LEFT) {
                if (player.hasItemInSlot(EquipmentSlot.OFFHAND)) {
                    x -= Constants.offhandSlotSize + Constants.bigPad; // Offhand slot + padding
                }
            }
            ...
            if (percentage > 0f) {
                int brightHeight = (int) (Constants.itemHeight * (1 - percentage));  // how much is filled
                guiGraphics.enableScissor(x, y + (Constants.itemHeight - brightHeight), x + Constants.itemWidth, y + Constants.itemHeight);
