...
public class AnnihilationSpellEntity extends BrutalityAbstractArrow implements BrutalityGeoEntity, IBrutalitySpellEntity {
    private static final EntityDataAccessor<Integer> SPELL_LEVEL_DATA = SynchedEntityData.defineId(AnnihilationSpellEntity.class, EntityDataSerializers.INT);

    public AnnihilationSpellEntity(EntityType<? extends AbstractArrow> pEntityType, Level pLevel) {
    ...
    public void tick() {
        super.tick();
        if (level() instanceof ClientLevel clientLevel && tickCount % 10 == 0 && !inGround) {
            clientLevel.addParticle(TerramityModParticleTypes.STYGIAN_PARTICLE.get(), this.getX(), this.getY(), this.getZ(), 0.0F, 0.0F, 0.0F);
        }
