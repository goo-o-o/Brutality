...
import net.goo.brutality.common.entity.projectile.generic.Bloodslash;
...
public class CrimsonScissorBlade extends BrutalitySwordItem {
    ...
    public Multimap<Attribute, AttributeModifier> getAttributeModifiers(EquipmentSlot slot, ItemStack stack) {
        ...
        if (slot == EquipmentSlot.MAINHAND || slot == EquipmentSlot.OFFHAND) {
            ...
            newAttributes.put(BrutalityAttributes.OMNIVAMP.get(),
                    new AttributeModifier(CSB_OMNIVAMP_UUID, "Omnivamp buff", 0.15, AttributeModifier.Operation.ADDITION));
    ...
    public void performBloodSlash(Player player) {
        ...
        Bloodslash bloodslash = new Bloodslash(BrutalityEntities.BLOODSLASH.get(), level);
        ...
        player.level().playSound(null, player.blockPosition(), BrutalitySounds.BLOOD_MAGIC_MISSILE.get(), SoundSource.PLAYERS, 1, Mth.nextFloat(player.getRandom(), 0.8F, 1.2F));
    ...
    public boolean hurtEnemy(ItemStack pStack, LivingEntity pTarget, LivingEntity pAttacker) {
        for (int i = 0; i < 10; i++) {
            pTarget.level().addParticle(BrutalityParticles.BLOOD_PARTICLE.get(), pTarget.getX(), pTarget.getY(0.5), pTarget.getZ(), 0, 0, 0);
        }
