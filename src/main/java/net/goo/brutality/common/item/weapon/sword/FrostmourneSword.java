...
public class FrostmourneSword extends BrutalitySwordItem {
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (!pPlayer.isShiftKeyDown()) {
            ...
            pLevel.playSound(pPlayer, pPlayer.getOnPos(), BrutalitySounds.ICE_WAVE.get(), SoundSource.PLAYERS, 1,
    ...
    public void performSoulrendAttack(Level level, Player player, ItemStack stack) {
        if (level instanceof ServerLevel serverLevel) {
            ...
            serverLevel.sendParticles(
                    new WaveParticleData<>(BrutalityParticles.FROSTMOURNE_WAVE.get(), 15, 80),
                    player.getX(),
            ...
            List<? extends Entity> entities = ModUtils.applyWaveEffect(serverLevel, player.getX(), player.getY(0.33F), player.getZ(), Entity.class,
                    new WaveParticleData<>(BrutalityParticles.FROSTMOURNE_WAVE.get(), 15, 80), e -> e != player, entity -> {
