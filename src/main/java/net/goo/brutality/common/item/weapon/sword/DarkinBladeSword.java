...
public class DarkinBladeSword extends BrutalitySwordItem {
        ...
        if (isSelected) {
            if (data.passiveCD > 0) {
                ...
                level.playSound(
                        null,
                        entity.getOnPos(),
                        BrutalitySounds.DEATHBRINGER_STANCE_READY.get(),
                        SoundSource.PLAYERS,
                        1.0F,
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity pTarget, LivingEntity pAttacker) {
        ...
        if (data != null && data.passiveCD <= 0) {
            ...
            if (pAttacker instanceof Player player)
                player.level().playSound(null, player.getOnPos(), BrutalitySounds.DEATHBRINGER_STANCE_HIT.get(),
    ...
    public boolean onEntitySwing(ItemStack stack, LivingEntity entity) {
        entity.level().playSound(null, entity.blockPosition(), ModUtils.getRandomSound(BrutalitySounds.DARKIN_BLADE), SoundSource.PLAYERS, 1, Mth.nextFloat(entity.level().getRandom(), 0.5F, 1.25F));
