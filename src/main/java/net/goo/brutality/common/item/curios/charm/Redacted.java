...
public class Redacted extends BrutalityCurioItem {
    public Redacted(Rarity rarity, List<ItemDescriptionComponent> descriptionComponents) {
    ...
    public static <T extends Entity> boolean shouldRedact(T entity) {
        if (entity instanceof LivingEntity living) {
            return CuriosApi.getCuriosInventory(living)
                    .map(handler -> handler.isEquipped(BrutalityItems.REDACTED.get()))
                    .orElse(false);
        }
        return false;
    }
