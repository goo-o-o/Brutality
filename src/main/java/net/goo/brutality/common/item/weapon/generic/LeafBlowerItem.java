...
public class LeafBlowerItem extends BrutalityGenericItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (pLevel.isClientSide) {
            ...
            pPlayer.playSound(BrutalitySounds.LEAF_BLOWER_ON.get(), 0.25F, 1);
        }
        if (!pLevel.isClientSide) {
    ...
    public void releaseUsing(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity, int pTimeCharged) {
         if (pLivingEntity instanceof Player pPlayer) {
                pPlayer.playSound(BrutalitySounds.LEAF_BLOWER_OFF.get(), 0.25F, 1);
             tickCounter = 0;
            }
    }
        ...
        if (isActive(pStack)) {
            if (pLevel.isClientSide && pEntity instanceof Player pPlayer) {
                ...
                if (tickCounter >= 28 && tickCounter % 5 == 0) {
                    pPlayer.playSound(BrutalitySounds.LEAF_BLOWER_ACTIVE.get(), 0.25F, 1);
                }
            }
