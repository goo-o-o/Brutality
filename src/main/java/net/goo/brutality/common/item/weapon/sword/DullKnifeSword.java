...
public class DullKnifeSword extends BrutalitySwordItem {
    ...
    public enum EmotionColor {
        NEUTRAL(new Color(255, 255, 255), new Color(34, 34, 34), BrutalityEffects.NEUTRAL.get()),
        HAPPY(new Color(255, 226, 0), new Color(255, 48, 251), BrutalityEffects.HAPPY.get()),
        SAD(new Color(0, 26, 127), new Color(0, 242, 255), BrutalityEffects.SAD.get()),
        ANGRY(new Color(255, 201, 0), new Color(215, 0, 0), BrutalityEffects.ANGRY.get());
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        pLevel.playSound(pPlayer, pPlayer.getOnPos(), BrutalitySounds.DULL_KNIFE_ABILITY.get(), SoundSource.PLAYERS);
    ...
    public boolean hurtEnemy(ItemStack pStack, LivingEntity pTarget, LivingEntity pAttacker) {
        if (pAttacker instanceof Player player)
            pAttacker.level().playSound(player, player.getOnPos(), BrutalitySounds.DULL_KNIFE_STAB.get(), SoundSource.PLAYERS);
    ...
    public boolean onEntitySwing(ItemStack stack, LivingEntity entity) {
        if (!ModList.get().isLoaded("bettercombat") && entity instanceof Player player) {
            player.level().playSound(player, player.getOnPos(), BrutalitySounds.DULL_KNIFE_SWING.get(), SoundSource.PLAYERS,
