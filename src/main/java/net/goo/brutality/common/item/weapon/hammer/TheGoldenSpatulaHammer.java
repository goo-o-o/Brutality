...
public class TheGoldenSpatulaHammer extends BrutalityHammerItem implements BrutalityGeoItem {
    ...
    public TheGoldenSpatulaHammer(Tier pTier, int pAttackDamageModifier, float pAttackSpeedModifier, Rarity rarity, List<ItemDescriptionComponent> descriptionComponents) {
    ...
    public boolean hurtEnemy(ItemStack pStack, @NotNull LivingEntity pTarget, @NotNull LivingEntity pAttacker) {
        pAttacker.level().playSound(null, pAttacker.getOnPos(), ModUtils.getRandomSound(BrutalitySounds.SPATULA), SoundSource.PLAYERS);
        pTarget.push(0D, 0.5D, 0D);

        pTarget.addEffect(new MobEffectInstance(BrutalityEffects.MASHED.get(), 100, 1, false, true));
        pTarget.addEffect(new MobEffectInstance(BrutalityEffects.SCORED.get(), 100, 1, false, true));
