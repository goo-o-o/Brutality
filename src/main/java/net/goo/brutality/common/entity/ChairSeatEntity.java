package net.goo.brutality.common.entity;
...
public class ChairSeatEntity extends Entity implements BrutalityGeoEntity {
    ...
    public ChairSeatEntity(Level level, BlockPos pos) {
        this(BrutalityEntities.CHAIR_SEAT.get(), level);
    ...
    public void tick() {
        ...
        if (!level().isClientSide && tickCount % 10 == 0) {
            ...
            if (!(state.is(BrutalityBlocks.WHITE_OFFICE_CHAIR.get()) || state.is(BrutalityBlocks.BLACK_OFFICE_CHAIR.get()) || state.is(BrutalityBlocks.TOILET.get()))) {
                this.discard();
            }
