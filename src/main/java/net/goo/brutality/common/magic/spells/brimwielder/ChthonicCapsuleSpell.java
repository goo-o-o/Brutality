...
public class ChthonicCapsuleSpell extends BrutalitySpell {
    ...
    public ChthonicCapsuleSpell() {
        super(MagicSchool.BRIMWIELDER,
                ...
                "chthonic_capsule",
                30, 5, 80, 0, 1, List.of(
                        new SpellTooltipRenderer.SpellStatComponent(SpellTooltipRenderer.SpellStatComponentType.SIZE, 3, 1, 3F, 50F)
                ));
    }
    ...
    public boolean onStartCast(Player player, ItemStack stack, int spellLevel) {
        if (player.level() instanceof ServerLevel serverLevel) {
            ChthonicCapsuleEntity chthonicCapsuleSpell = new ChthonicCapsuleEntity(BrutalityEntities.CHTHONIC_CAPSULE_ENTITY.get(), serverLevel);
