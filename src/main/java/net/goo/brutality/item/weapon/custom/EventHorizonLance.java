...
public class EventHorizonLance extends BrutalityTridentItem {

    public EventHorizonLance(Properties pProperties, float attackDamageModifier, float attackSpeedModifier, String identifier, Rarity rarity, List<BrutalityTooltipHelper.DescriptionComponent> descriptionComponents) {
    ...
    public void spawnBlackHole(Player player, ItemStack pStack) {
        if (player.level() instanceof ServerLevel serverLevel) {
            serverLevel.playSound(null, player.getOnPos(), ModSounds.EVENT_HORIZON_DISLODGE.get(), SoundSource.AMBIENT);

            BlackHoleEntity blackHole = new BlackHoleEntity(ModEntities.BLACK_HOLE_ENTITY.get(), serverLevel);
    ...
    public void despawnBlackHole(Player player, ItemStack pStack) {
        if (player.level() instanceof ServerLevel serverLevel) {
            serverLevel.playSound(null, player.getOnPos(), ModSounds.EVENT_HORIZON_RETURN.get(), SoundSource.AMBIENT);

            BlackHoleEntity blackHole = getOwnedBlackHoleFromID(player, pStack);
            if (blackHole != null) {
                blackHole.discard();
            }
    ...
    private BlackHoleEntity getOwnedBlackHoleFromID(Player player, ItemStack pStack) {
        Entity blackHole = player.level().getEntity(pStack.getOrCreateTag().getInt("ID"));
        if (blackHole instanceof BlackHoleEntity) {
            return (BlackHoleEntity) blackHole;
        }
        return null;
    }
