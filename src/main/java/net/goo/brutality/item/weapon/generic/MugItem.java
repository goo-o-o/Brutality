...
public class MugItem extends BrutalityGenericItem {
    public MugItem(Rarity rarity, List<BrutalityTooltipHelper.ItemDescriptionComponent> descriptionComponents) {
    ...
    private PlayState predicate(AnimationState<MugItem> mugItemAnimationState) {
        ItemStack stack = mugItemAnimationState.getData(DataTickets.ITEMSTACK);
        if (ModUtils.getTextureIdx(stack) == 2) {
            mugItemAnimationState.getController().setAnimation(RawAnimation.begin().thenPlay("eeffoc"));
            ...
            if (ModUtils.getTextureIdx(pStack) == 0) { // Throwing
                if (i >= 10) {
                    if (!pLevel.isClientSide) {
                        launchProjectile(pLevel, player);
