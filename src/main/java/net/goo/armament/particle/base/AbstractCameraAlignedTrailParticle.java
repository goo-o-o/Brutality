...
public class AbstractCameraAlignedTrailParticle extends Particle {
    ...
    public AbstractCameraAlignedTrailParticle(ClientLevel world, double x, double y, double z, double xd, double yd, double zd, float r, float g, float b) {
        super(world, x, y, z);
        this.xd = xd;
        this.yd = yd;
        this.zd = zd;
    ...
    public void render(@NotNull VertexConsumer consumer, @NotNull Camera camera, float partialTick) {
        if (trailPointer > -1) {
            ...
            Vec3 topOffset = new Vec3(0, 10 / 2F, 0); // Flat plane (Y-axis up)
            Vec3 bottomOffset = new Vec3(0, -10 / 2F, 0);
    ...
    protected ResourceLocation getTrailTexture() {
        return new ResourceLocation(Armament.MOD_ID, "textures/particle/circle_trail_particle.png");
    }
