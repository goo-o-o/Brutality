...
public abstract class ArmaArrow extends Projectile implements ArmaGeoEntity {
    ...
    protected ArmaArrow(EntityType<? extends ArmaArrow> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    protected ArmaArrow(EntityType<? extends ArmaArrow> pEntityType, double pX, double pY, double pZ, Level pLevel) {
        this(pEntityType, pLevel);
        this.setPos(pX, pY, pZ);
    }

    protected ArmaArrow(EntityType<? extends ArmaArrow> pEntityType, LivingEntity pShooter, Level pLevel) {
    ...
    public double getGravity() { return 0.05F; }

    public SimpleParticleType getFlightParticle() { return ParticleTypes.CRIT; }
    ...
    public void tick() {
        ...
        if (this.inGround && !flag) {
            ...
            if (this.isCritArrow()) {
                for(int i = 0; i < 4; ++i) {
                    this.level().addParticle(getFlightParticle(), this.getX() + d5 * (double)i / 4.0D, this.getY() + d6 * (double)i / 4.0D, this.getZ() + d1 * (double)i / 4.0D, -d5, -d6 + 0.2D, -d1);
                }
            }
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (entity1 == null) {
            ...
            damagesource = this.damageSources().playerAttack(((Player) entity1));
    ...
    public void setOwner(@Nullable Entity pEntity) {
        ...
        if (pEntity instanceof Player) {
            this.pickup = ((Player)pEntity).getAbilities().instabuild ? Pickup.CREATIVE_ONLY : Pickup.ALLOWED;
        }
